...
enum { kMindDataTypeBlink = 0x16 };
enum {
    kRawBlinkWindowSize = 64,
    kRawBlinkPeakToPeakThreshold = 900,
    kRawBlinkSlopeThreshold = 650,
    kRawBlinkRefractoryMs = 550
};
static int tgRawCallbackCount = 0;
static int tgOtherCallbackCount = 0;
static int tgBlinkCount = 0;
static int tgLastBlinkStrength = 0;
static int rawBlinkWindow[kRawBlinkWindowSize] = {0};
static int rawBlinkWindowIndex = 0;
static int rawBlinkWindowCount = 0;
static int rawBlinkLastSample = 0;
static BOOL rawBlinkHasLastSample = NO;
static long long rawBlinkLastDetectedMs = 0;
...
/*    plotAvaliable graphTitle                  plotName                                                        plots   colors  xRange          plotMinY    plotMaxY    setting*/
    ...
    { YES,          @"EEG Raw",                 {@"EEG Raw",     nil},                                          {nil},  {nil},  X_RANGE,        -2048,      4096,       defaultAlgoSetting[SegmentEEGRaw] },
...
- (void)viewDidLoad {
    ...
    [[TGStreamForMac sharedInstance:kTGAMSerialPortPath] setDelegate:self];
...
- (void)showBlinkWithSource:(NSString *)source strength:(int)strength {
    tgBlinkCount++;
    tgLastBlinkStrength = strength;
    NSLog(@"Blink #%d detected by %@, strength=%d", tgBlinkCount, source, strength);

    dispatch_async(dispatch_get_main_queue(), ^{
        [eyeBlinkImage setImage:[NSImage imageNamed:@"led-on"]];
        bBlink = YES;
        [stateLabel setStringValue:[NSString stringWithFormat:@"BLINK #%d (%@) strength: %d", tgBlinkCount, source, strength]];
        [signalLabel setStringValue:[NSString stringWithFormat:@"TG raw callbacks: %d | blinks: %d last: %d", tgRawCallbackCount, tgBlinkCount, tgLastBlinkStrength]];
        [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(eyeBlinkAnimate) userInfo:nil repeats:NO];
    });
}

- (void)detectRawBlinkFromSample:(int)sample {
    int slope = 0;
    if (rawBlinkHasLastSample) {
        slope = sample - rawBlinkLastSample;
        if (slope < 0) {
            slope = -slope;
        }
    }
    rawBlinkLastSample = sample;
    rawBlinkHasLastSample = YES;

    rawBlinkWindow[rawBlinkWindowIndex] = sample;
    rawBlinkWindowIndex = (rawBlinkWindowIndex + 1) % kRawBlinkWindowSize;
    if (rawBlinkWindowCount < kRawBlinkWindowSize) {
        rawBlinkWindowCount++;
        return;
    }

    int minValue = rawBlinkWindow[0];
    int maxValue = rawBlinkWindow[0];
    for (int i = 1; i < kRawBlinkWindowSize; i++) {
        if (rawBlinkWindow[i] < minValue) {
            minValue = rawBlinkWindow[i];
        }
        if (rawBlinkWindow[i] > maxValue) {
            maxValue = rawBlinkWindow[i];
        }
    }

    int range = maxValue - minValue;
    long long now = current_timestamp();
    BOOL refractoryPassed = (now - rawBlinkLastDetectedMs) > kRawBlinkRefractoryMs;
    BOOL blinkCandidate = (range >= kRawBlinkPeakToPeakThreshold && slope >= kRawBlinkSlopeThreshold);

    if (refractoryPassed && blinkCandidate) {
        rawBlinkLastDetectedMs = now;
        [self showBlinkWithSource:@"raw EEG threshold" strength:range];
    }
}

- (void)eyeBlinkDetect:(NSNumber *)strength {
    [self showBlinkWithSource:@"NskAlgoSdk" strength:[strength intValue]];
}

- (void)eegBlink:(int)blinkValue {
    [self showBlinkWithSource:@"TGStreamMac" strength:blinkValue];
}
...
- (IBAction)openConnectionAction:(id)sender {
    [stateLabel setStringValue:[NSString stringWithFormat:@"Opening %@", kTGAMSerialPortPath]];
    [[TGStreamForMac sharedInstance:kTGAMSerialPortPath] startStream];
    
    [self headsetConnection:YES];
    [segmentControl setSelected:YES forSegment:SegmentEEGRaw];
...
-(void)onDataReceived:(NSInteger)datatype data:(int)data obj:(NSObject *)obj {
    if (datatype != MindDataType_CODE_RAW) {
        tgOtherCallbackCount++;
        if (tgOtherCallbackCount <= 20 || tgOtherCallbackCount % 50 == 0) {
            NSLog(@"TGStream data type=%ld data=%d obj=%@", (long)datatype, data, obj);
        }
    }
    ...
    switch (datatype) {
        ...
        case MindDataType_CODE_RAW: {
            rawCount++;
            tgRawCallbackCount++;
            [self detectRawBlinkFromSample:data];

            if (tgRawCallbackCount <= 10 || tgRawCallbackCount % 128 == 0) {
                NSLog(@"TGStream raw callback count=%d latest=%d", tgRawCallbackCount, data);
                dispatch_async(dispatch_get_main_queue(), ^{
                    [signalLabel setStringValue:[NSString stringWithFormat:@"TG raw callbacks: %d latest: %d | blinks: %d last: %d", tgRawCallbackCount, data, tgBlinkCount, tgLastBlinkStrength]];
                });
            }
        ...
        case kMindDataTypeBlink:
            [self showBlinkWithSource:@"TGStream packet" strength:data];
            break;
...
-(void) onChecksumFail:(Byte *)payload length:(NSUInteger)length checksum:(NSInteger)checksum{
    checkSum++;
    if (checkSum <= 10 || checkSum % 20 == 0) {
        NSLog(@"TGStream checksum failures: %lu", (unsigned long)checkSum);
    }
...
-(void)onStatesChanged:(ConnectionStates)connectionState{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [stateLabel setStringValue:[NSString stringWithFormat:@"Stream: %@", connectState]];
    });
